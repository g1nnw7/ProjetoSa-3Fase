...
function CalculatorIcon({ className }) {
  return (
    ...
    >
      ...
      <path strokeLinecap="round" strokeLinejoin="round" d="M8 6h8m0 8v4m0-8h.01M12 10h.01M8 10h.01M12 14h.01M8 14h.01M12 18h.01M8 18h.01" />
    </svg>
  );
}
...
function AddFoodModal({ onClose, onSave }) {
    ...
    const handleSubmit = async (e) => {
        ...
        const foodToSave = {
            nome: newFood.nome,
            gramas: parseFloat(newFood.gramas),
            calorias: parseFloat(newFood.calorias),
            proteinas: parseFloat(newFood.proteinas),
            carboidratos: parseFloat(newFood.carboidratos),
            gorduras: parseFloat(newFood.gorduras)
        };

        try {
            await onSave(foodToSave);
            onClose();
        } catch (error) {
            console.error(error);
        } finally {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
                <div className="flex justify-between items-center p-4 border-b border-gray-100 bg-gray-50">
                    <h3 className="font-bold text-green-500 text-lg">Cadastrar Novo Alimento</h3>
                    <button onClick={onClose} className="text-gray-500 hover:text-gray-700 p-1 rounded-full hover:bg-gray-200 transition-colors cursor-pointer">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Porção Base (g)</label>
                            <input 
                                type="number" 
                                name="gramas"
                                value={newFood.gramas} 
                                onChange={handleChange}
                                required
                                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none"
                            />
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-blue-600 mb-1 uppercase">Proteínas</label>
                            <input 
                                type="number" 
                                name="proteinas"
                                value={newFood.proteinas} 
                                onChange={handleChange}
                                required
                                step="0.1"
                        ...
                        <div>
                            <label className="block text-xs font-bold text-yellow-600 mb-1 uppercase">Carboidratos</label>
                            <input 
                                type="number" 
                                name="carboidratos"
                                value={newFood.carboidratos} 
                                onChange={handleChange}
                                required
                                step="0.1"
                        ...
                        <div>
                            <label className="block text-xs font-bold text-red-600 mb-1 uppercase">Gorduras</label>
                            <input 
                                type="number" 
                                name="gorduras"
                                value={newFood.gorduras} 
                                onChange={handleChange}
                                required
                                step="0.1"
                    ...
                    <button 
                        type="submit"
                        disabled={loading}
                        className="w-full mt-4 bg-green-600 text-white py-3 rounded-lg font-semibold hover:bg-green-700 transition-colors disabled:bg-gray-400 shadow-md cursor-pointer"
                    >
...
export default function MacroCalculadora() {
  ...
  const handleSubmit = (e) => {
    ...
    if (!alimentoBase) {
      toast.error('Alimento não encontrado! Adicione-o à lista ou selecione outro.');
      return;
    }
  ...
  const handleSaveNewFood = async (foodData) => {
      try {
          const response = await axios.post('http://localhost:3000/alimentos', foodData);
          setAlimentosDB([...alimentosDB, response.data]);
          setAlimentoInput(response.data.nome); 
      ...
      <div className="container mx-auto max-w-5xl p-4 md:p-8">
        ...
        <div className="mb-8 p-6 bg-white rounded-xl shadow-sm border border-gray-200">
          ...
          <p className="text-gray-600 mt-2">
            Planeje sua dieta calculando calorias e macronutrientes. Os dados ficam salvos até você sair da conta.
          </p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="w-full lg:w-1/3">
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 sticky top-6 z-20">
              <h2 className="text-xl font-semibold text-gray-800 mb-4 border-b pb-2">Adicionar à Refeição</h2>
              ...
              <div className="mt-6 pt-4 border-t border-gray-100">
                  <p className="text-xs text-gray-500 flex flex-col items-center justify-center text-center gap-1">
                      <span>Não encontrou seu alimento?</span>
                      <button 
                        type="button" 
                        onClick={() => setShowAddModal(true)}
                        className="text-green-600 hover:text-green-800 font-semibold hover:underline transition-colors cursor-pointer flex items-center"
                      >
          ...
          <div className="w-full lg:w-2/3 z-10">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                 <h2 className="text-xl font-semibold text-green-500">Refeição Atual</h2>
