...
function EmptyCartIcon() {
  return <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-24 h-24 text-gray-300"><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25 5.106 5.272A1.125 1.125 0 0 1 6.182 4h11.636a1.125 1.125 0 0 1 1.076 1.272L19.5 14.25m-12 0h12" /></svg>;
}
          ...
          {cartState.items.length > 0 && (
            <div className="p-4 border-t bg-gray-50">
              ...
              <div className="mb-4 border-b border-gray-200 pb-4">
                ...
                {cepError && <p className="text-red-500 text-xs mt-1 font-medium">{cepError}</p>}
              ...
              <div className="space-y-1 mb-4 text-sm text-gray-600">
                ...
                {discount > 0 && <div className="flex justify-between text-green-600 font-medium"><span>Desconto:</span><span>- R$ {discount.toFixed(2).replace('.', ',')}</span></div>}
                <div className="flex justify-between items-center pt-2 border-t border-gray-200 mt-2">
                    <span className="text-lg font-bold text-gray-900">Total:</span>
                    <span className="text-2xl font-extrabold text-green-700">R$ {finalTotal.toFixed(2).replace('.', ',')}</span>
                </div>
              </div>

              <button
                onClick={handleCheckout}
                className={`w-full py-3 px-4 rounded-lg text-white font-semibold text-lg hover:bg-green-700 transition-colors
              ...
              >
                {selectedShipping ? 'Finalizar Compra' : 'Selecione o Frete'}
              </button>
            </div>
          )}
        </div>
